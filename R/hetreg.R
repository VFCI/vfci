...
#'
#' `hetreg()` is a wrapper for [nlme::gls()]. Compared to [`gls()`][nlme::gls()],
#' the function `hetreg()`
...
#' a combination [`varComb()`][nlme::varComb()] of [varExp()][nlme::varExp()] terms
#' * has tidy support
#'
...
#'  variables that model the mean function; \eqn{v_i = (v_{1i},v_{2i},\dotsc,v_{mi})}
#'   are the m variables that model the variance function (corresponding to the
#'   argument `het`); \eqn{\beta} is a column vector of unknown parameters in the
#'    mean function, and \eqn{\theta} is a column vector of unknown
#'    parameters in the variance function.
#'  `hetreg()` is similar to the command [hetregress](https://www.stata.com/manuals/rhetregress.pdf)
#'  in Stata, where
#'  ```Stata
...
#' @param x vector of strings (character vectors) with the name of the independent variable(s).
#' @param het vector of strings (character vectors) with the name of the variable(s) that enter(s) the variance function as independent variable(s).
